#!/usr/bin/env python3
"""
Polymarket Data Collector (Gamma API)
Fetches event probabilities from Polymarket Gamma API (better filtering)
"""
...
def extract_event_data(market: Dict) -> Optional[Dict]:
    """
    Extract relevant fields from Polymarket market (Gamma API)
    ...
    try:
        ...
        if market.get("closed", True) or market.get("archived", False):
            return None
        
        # Get probability from outcomePrices (first outcome, usually "Yes")
        ...
        event = {
            ...
            "category": "Crypto",  # Simplified - would need better categorization
        ...
        print(f"⚠️  Parse error for {market.get('question', 'unknown')[:30]}: {e}")
        return None
...
def store_event(conn: sqlite3.Connection, event: Dict):
    ...
    if result:
        ...
        if magnitude > 0.01:
            ...
            direction = "📈" if new_prob > old_prob else "📉"
            print(f"{direction} {event['title'][:50]}: {old_prob:.1%} → {new_prob:.1%} "
                  f"({magnitude:+.1%})")
...
def run_continuous(interval_seconds: int = 60):
    ...
    """
    print(f"🚀 Starting Polymarket collector (Gamma API, interval: {interval_seconds}s)")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "once":
            ...
            print("Usage:")
            print("  python3 polymarket_collector_v2.py once           # Single snapshot")
            print("  python3 polymarket_collector_v2.py continuous [N] # Run every N seconds")
    else:
